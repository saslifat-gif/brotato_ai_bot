...
``v4.train_human_demo_bc``; this evaluator reproduces that script's seeded
...
def load_model(path: Path, width: int):
    ...
    if not isinstance(state_dict, dict):
        raise RuntimeError("expected the v4.train_human_demo_bc checkpoint format")
...
def main() -> int:
    ...
    split_documentation = {
        "method": "reproduces v4.train_human_demo_bc.py: sorted episode IDs, NumPy default_rng(seed=7), first floor(80%) train and remainder validation",
        "seed": args.seed,
