"""Run a short deterministic end-to-end v4 automation check without PPO."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke-test v4 combat and menu automation")
    ...
    try:
        ...
        print(f"[v4-smoke] reset phase={info['phase']} tick={info['tick']}")
        ...
        for step in range(max(1, args.steps)):
            ...
            if not phases or phases[-1] != phase:
                ...
                state = env.last_state or {}
                print(
                    f"[v4-smoke] step={step + 1} phase={phase} "
                ...
                state = env.last_state or {}
                print(
                    f"[v4-smoke] progress step={step + 1} wave={info.get('wave')} "
            ...
            if terminated:
                ...
                confirmed_ui.update(info.get("ui_confirmed", []))
                print(
                    f"[v4-smoke] episode_reset={episodes} "
        ...
        print(
            f"[v4-smoke] ok steps={completed_steps} episodes={episodes} "
