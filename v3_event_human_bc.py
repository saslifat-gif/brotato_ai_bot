...
def load_frames(path: Path) -> list[Frame]:
    ...
    records = connection.execute(
        ...
        SELECT frame_id,episode_id,frame_number,timestamp_ns,phase,wave,action,
               previous_action,feature_blob,state_blob,derived_blob
        FROM frames
...
def select_training_examples(examples: list[EventExample], train_indices: set[int], seed: int,
                             negative_ratio: int) -> list[EventExample]:
    ...
    weights = np.asarray([1.0 + example.hard_negative_score for example in negatives], dtype=np.float64)
    weights /= weights.sum()
    selected = rng.choice(len(negatives), size=target, replace=False, p=weights)
    values = positives + [negatives[int(index)] for index in selected]
    rng.shuffle(values)
    return values
...
def train_model(examples: list[EventExample], normalized: np.ndarray, selected_indices: list[int],
    ...
    for _ in range(max(1, epochs)):
        ...
        for start in range(0, len(order), max(16, batch_size)):
            ...
            loss = change_loss + action_loss + 0.15 * duration_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    model.eval()
